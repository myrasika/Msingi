...
exports.classifySituation = classifySituation;
exports.simplifyLegalText = simplifyLegalText;
exports.validateExplanation = validateExplanation;
exports.ensurePublishedExplanation = ensurePublishedExplanation;
...
const openaiClient = process.env.OPENAI_API_KEY
    ...
    : null;
const buildFallbackExplanation = (sections, title, language) => {
...
async function classifySituation(query) {
    return (0, searchService_1.searchSituations)(query);
}
async function simplifyLegalText(sections, language, situationTitle = 'this situation') {
...
async function validateExplanation(explanation, sections) {
    if (!explanation)
        return false;
    const combined = [
        explanation.summary ?? '',
...
async function ensurePublishedExplanation(situationId, language) {
    ...
    if (!situation)
        return null;
    const sections = situation.legalSections.map((item) => item.legalSection);
    const generated = await simplifyLegalText(sections, language, situation.title);
    return prisma_1.default.explanation.upsert({
